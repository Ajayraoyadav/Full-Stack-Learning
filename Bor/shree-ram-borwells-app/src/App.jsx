...
async function fetchWithRetry(url, options, maxRetries = 5) {
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            if (!response.ok) {
                // Catching API errors that return non-200 status
                const errorBody = await response.text();
                throw new Error(`HTTP error! status: ${response.status} - ${errorBody}`);
            }
            return response;
        } catch (error) {
            // This 'error' is the one the ESLint warning was referring to, but it's used now.
            if (i === maxRetries - 1) {
                throw error;
            }
        }
...
// --- Component Definitions (Moved to Top Level to fix 'create components during render' error) ---
...
function SOWGenerator({ projectName, isModalOpen, setIsModalOpen }) {
    ...
    // Renamed local variable 'error' to 'apiError' to resolve the ESLint warning 'err is defined but never used'
...
// 5. Expense Card Helper Component (MOVED HERE TO FIX ESLINT ERROR)
          ...
          {adminPage === 'dashboard' && (
            <div>
              ...
              <div className="border-t pt-4">
                  ...
                  <p className="text-sm text-gray-600 mb-4">
                      **Note on Saving:** Generating and saving reports automatically to your Gmail/Drive is a complex task requiring a secure backend server. This prototype provides an instant download of all current data (revenue and expenses) as a CSV file, which you can then easily upload to your Google Sheet or Excel for permanent archival.
                  </p>
                  <button
